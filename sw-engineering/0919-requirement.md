...
![](./resources/usecase-example.png)

![교재참고](./resources/IMG_20170928_113110.jpg)

### 사용 사례 관계 찾기

관계를 사용하여 모형의 복잡도를 줄여 이해도를 높임

- **포함**(include) - 사용 사례 사이의 공통된 동작(중복)을 제거
- **확장**(extend) - 정상적인 이벤트와 예외적인 이벤트를 분리
...
- 사용자와 개발자간의 이해를 돕기위해 작성한다.
- 시스템의 기능을 정확하고 완벽하며 일관성있게 작성한 것.
- 소프트웨어에 포함될 기능과 제약조건을 나열
- 기능에 대한 설명과 예외처리를 기술
- 시스템 성능과 관련된 사항, 속도, 정확성, 사용 용이성 포함

### 명세서 작성

- 요구분석서는 **사용자와 개발자 모두**가 **쉽게 이해**할 수 있도록 써야한다
- 요구 분석서에 기술된 조건은 개발자와 사용자가 모두 동의한 것이어야한다.
- 요구 분석서는 목표 시스템에 의하여 수행될 **모든 기능을 정확히 기술**하여야 한다.
- 요구 분석서는 목표 시스템에 영향을 주는 모든 **제약 조건을 기술**한다.
- 요구 분석서는 **시스템의 인수를 위한 테스트 기준을 제공**하여야 한다.
- 요구 분석서는 원하는 **시스템의 품질과 상대적인 중요도 및 품질을 재는 방법**이 기술 되어야 한다.

### 명세서 목차 예시

![교제참고](./resources/IMG_20170928_113813.jpg)

### 명세서 검토

- 무결성과 완벽성 - 요구분석서는 사용자의 요구를 오류없이 완벽하게 반영
- 일관성 - 요구분석서에는 서로 **모순**되는 부분이 없어야 함
- 명확성 - 요구분석서의 내용이 여러 의미로 해석되는 모호한 점이 없는지 확인
- 기능적 - 어떻게 보다 **무엇을** 에 관점
- 검증 가능성
  - 사용자 요구 만족
  - 시스템이 요구번석에 기술된 내용가 일치하는지 확인
- 추적가능성과 변경용이성 - 내용은 체계적으로 정리되어야 한다.
