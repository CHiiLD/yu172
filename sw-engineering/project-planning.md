...
- WBS를 작성하고 일정, 인력, 비용 등을 예측한다.

- 각 단계별 위험 관리와 품질 관리 그리고 형상 관리 등의 관리 프로세스를 계획한다.

- 예측된 기능적 요구의 구현가능성을 파악하기 위해 프로토 타입 작성

  ![t검증](./resources/plan-proto.png)

#### 4.1.2 요구분석

- 고객의 요구를 면밀히 분석하여 요구 도출한다.
- 분석된 기능적 요구의 실현가능성을 파악하기 위해 프로토 타입 작성한다.

#### 4.1.3 설계

- 아키텍처와 모듈, UI를 설계한다.
- 디자인패턴과 클래스, 인터페이스 등의 상세 설계
- 협업을 위해 사전에 코딩 규약, 문서 포맷을 선택한다.

#### 4.1.4 구현과 테스트

- 이전 단계에서 작성된 상세 설계를 중심으로 서로 독립된 시스템의 기능부터 구현한다.
- 테스트와 구현을 동시에 작업하여 높은 품질의 코드를 생산한다.
- 상호작용이 있는 모듈들의 구현이 완료되었을 때, 모듈간 통합 테스트를 수행한다.
- Doxygen을 이용하여 API 문서를 자동 생성한다.
...
| 요구분석     | 요구사항 정의<br>요구분석서 작성                      |
| 설계       | 시스템 구조 설계<br>UI 설계<br>상세 설계<br>설계서 작성<br>코딩 규약, 문서 가이드 제작 |
